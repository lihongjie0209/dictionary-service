...
import (
	...
	docs "github.com/lihongjie0209/dictionary-service/docs"
	"github.com/lihongjie0209/dictionary-service/internal/auth"
	"github.com/lihongjie0209/dictionary-service/internal/buildinfo"
	"github.com/lihongjie0209/dictionary-service/internal/config"
	"github.com/lihongjie0209/dictionary-service/internal/health"
	"github.com/lihongjie0209/dictionary-service/internal/observability"
	"github.com/lihongjie0209/dictionary-service/internal/ratelimit"
...
func NewServer(lc fx.Lifecycle, cfg config.Config, handler *Handler, authService *auth.Service, limiter *ratelimit.Limiter, metrics *observability.Metrics, tracing *observability.Tracing, logger *slog.Logger) (*http.Server, error) {
	...
	api.POST("/dictionaries/create", handler.CreateDictionary)
	api.POST("/dictionaries/update", handler.UpdateDictionary)
	api.POST("/dictionaries/get", handler.GetDictionary)
	...
	api.POST("/dictionaries/tree", handler.TreeDictionary)
	api.POST("/dictionaries/resolve", handler.ResolveCodes)
	api.POST("/dictionaries/providers/register", handler.RegisterProvider)
	api.POST("/dictionaries/providers/heartbeat", handler.HeartbeatProvider)
	api.POST("/dictionaries/providers/unregister", handler.UnregisterProvider)
	api.POST("/dictionaries/providers/list", handler.ListProviders)
