...
func setDefaults(v *viper.Viper) {
	v.SetDefault("app.name", "dictionary-service")
	...
	v.SetDefault("database.schema", "dictionary")
	...
	v.SetDefault("jwt.issuer", "dictionary-service")
	...
	v.SetDefault("auth.audience", "dictionary-service")
	...
	v.SetDefault("migration.table", "dictionary_schema_migrations")
	...
	v.SetDefault("provider_client.enabled", false)
	v.SetDefault("provider_client.allowed_dns_suffixes", []string{"-service", ".svc", ".svc.cluster.local"})
	v.SetDefault("provider_client.allow_insecure", false)
	v.SetDefault("provider_client.retry.max_attempts", 2)
	v.SetDefault("provider_client.retry.initial_backoff", "100ms")
	v.SetDefault("provider_client.retry.max_backoff", "1s")
	v.SetDefault("provider_client.retry.methods", []string{"/platform.dictionary.v1.DictionaryProviderService/*"})
...
func (c Config) Validate() error {
	...
	if c.ProviderClient.Enabled {
		if len(c.ProviderClient.AllowedDNSSuffixes) == 0 || len(c.ProviderClient.PSK) < 32 {
			return errors.New("enabled provider_client requires allowed DNS suffixes and a PSK of at least 32 bytes")
		}
		if c.App.Env == "production" && (!c.ProviderClient.TLS.Enabled || c.ProviderClient.AllowInsecure) {
			return errors.New("production provider_client requires TLS and forbids insecure credentials")
		}
		if !c.ProviderClient.TLS.Enabled && !c.ProviderClient.AllowInsecure {
			return errors.New("provider_client without TLS requires explicit allow_insecure")
		}
		auth := ClientAuth{}
		if c.ProviderClient.TLS.Enabled {
			auth = ClientAuth{Type: "psk", Token: c.ProviderClient.PSK}
		}
		if err := validateClientPolicy("provider_client", auth, c.ProviderClient.Retry, c.ProviderClient.Breaker, c.ProviderClient.TLS, c.App.Env == "production"); err != nil {
			return err
		}
	}
