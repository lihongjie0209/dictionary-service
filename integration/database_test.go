...
import (
	...
	appdb "github.com/lihongjie0209/dictionary-service/internal/database"
	"github.com/lihongjie0209/dictionary-service/internal/migration"
...
func TestRepositoryAndMigrations(t *testing.T) {
	for _, databaseType := range []string{"postgres", "mysql"} {
		t.Run(databaseType, func(t *testing.T) {
			...
			var dictionaryTables int
			if databaseType == "postgres" {
				if err := db.GetContext(ctx, &dictionaryTables, `SELECT count(*) FROM pg_tables WHERE schemaname = current_schema() AND tablename IN ('dictionaries','dictionary_item_drafts','dictionary_releases','dictionary_release_items','dictionary_providers','dictionary_outbox_events')`); err != nil {
					t.Fatal(err)
				}
			} else if err := db.GetContext(ctx, &dictionaryTables, `SELECT count(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name IN ('dictionaries','dictionary_item_drafts','dictionary_releases','dictionary_release_items','dictionary_providers','dictionary_outbox_events')`); err != nil {
				t.Fatal(err)
			}
